...
const Videos = () => {
  ...
  direction={["column","row"]} height={"100vh"}>
    ...
    <VStack w={"full"} alignItems={"flex-start"} p={12} overflowY={"auto"} >
        <video 
               controls
        ...
        <Text>This is the sample video 1</Text>
    </VStack>

    <VStack
    w={["full","xl"]} alignItems={"stretch"} p={8} spacing={12} overflowY={"auto"}>
        {
            videosArr.map((item,index)=>(
            <Button variant={"ghost"} colorScheme='red' onClick={()=>setVideoSrc(item)}>
                Video {index + 1 }
                </Button>
            ))
        }
